...
export const ChatAside = () => {
  const setIsLeftAsideCollapsed = useUIStore(
    (state) => state.setIsLeftAsideCollapsed
  );
  const isLeftAsideCollapsed = useUIStore(
    (state) => state.isLeftAsideCollapsed
  );

  const [isModalUploadDocumentOpen, setIsModalUploadDocumentOpen] =
  ...
  const handleOpenModalUploadDocument = () => {
    if (!isModalUploadDocumentOpen) setIsModalUploadDocumentOpen(true);
  };

  const handleCloseUploadDocumentModal = () => {
    if (isModalUploadDocumentOpen) setIsModalUploadDocumentOpen(false);
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          className="avara-title text-[20px] leading-6 font-[var(--font-exo2)] text-black"
          hidden={isLeftAsideCollapsed}
        >
          AVARA LABS
        </span>
        ...
        <div className="flex flex-grow justify-end items-center">
          {!isLeftAsideCollapsed && (
            <button className="cursor-pointer" onClick={handleCollapsed}>
              <ChevronLeft className="text-black" size={32}></ChevronLeft>
            </button>
          )}
          {isLeftAsideCollapsed && (
            <button className="cursor-pointer" onClick={handleCollapsed}>
              <ChevronRight className="text-black" size={32}></ChevronRight>
            </button>
          )}
        </div>
      </div>
      ...
      >
        <InputButton
          showOnlyIcon={isLeftAsideCollapsed}
          text="Upload"
          className="w-full"
          icon={<Upload></Upload>}
